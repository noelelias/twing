 ...
 * @param {Promise<*> | *} value The value to be escaped
 * @param {string} strategy The escaping strategy
 * @param {string} charset The charset
 * @param {boolean} autoescape Whether the function is called by the auto-escaping feature (true) or explicitly (false)
 *
 * @returns {Promise<string>}
 */
export function twingFilterEscape(env: TwingEnvironment, value: Promise<any> | any, strategy: string = 'html', charset: string = null, autoescape: boolean = false): Promise<string> {
    return Promise.resolve(value)
        .then((value) => {
            if (autoescape && value && value instanceof TwingMarkup) {
                return value;
            }

            if (typeof value !== 'string') {
                if (value && (typeof value === 'object') && Reflect.has(value, 'toString')) {
                    value = '' + value;
            ...
            switch (strategy) {
                ...
                case 'js':
                    value = value.replace(/[^a-zA-Z0-9,\._]/ug, function (matches: string) {
                         ...
                         * \" is also supported but omitted, because the resulting value is not HTML safe.
                         */
                    ...
                    return value;
                case 'css':
                    value = value.replace(/[^a-zA-Z0-9]/ug, function (matches: string) {
                        let char = matches;
                ...
                default:
                    ...
                    if (escapers.has(strategy)) {
                        return escapers.get(strategy)(env, value, charset);
                    }

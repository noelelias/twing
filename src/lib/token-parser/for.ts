...
export class TwingTokenParserFor extends TwingTokenParser {
    parse(token: TwingToken) {
        ...
        if (stream.nextIf(TwingToken.NAME_TYPE, 'if')) {
            console.error(`Using an "if" condition on "for" tag in "${stream.getSourceContext().getName()}" at line ${lineno} is deprecated since Twig 2.10.0, use a "filter" filter or an "if" condition inside the "for" body instead (if your condition depends on a variable updated inside the loop).`);
